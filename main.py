...
# 示例数据，您可以替换为从Excel或其他来源加载的数据
data = {
    'time': pd.date_range(start='2022-01-01', end='2022-12-31', freq='D'),
    'category': ['A', 'B', 'C', 'D'] * 91 + ['A'],
    'value': range(365),
    'name': ['Alice', 'Bob', 'Charlie', 'David'] * 91 + ['Alice']
}
...
# 日期选择
st.subheader("选择开始与结束的日期 (区间:2022-01-01 至 2022-12-31)")
start_date = st.date_input('选择开始日期', datetime.date(2022, 1, 1))
end_date = st.date_input('选择结束日期', datetime.date(2022, 12, 31))

# 类别选择
st.subheader("选择类别")
categories = st.multiselect('选择一个或多个类别', ['A', 'B', 'C', 'D'], default=['A', 'B', 'C', 'D'])

# 数值范围选择
st.subheader("选择数值范围")
value_range = st.slider('选择数值范围', min_value=int(df_original['value'].min()), max_value=int(df_original['value'].max()), value=(int(df_original['value'].min()), int(df_original['value'].max())))

# 单选按钮
st.subheader("选择名称")
name_option = st.radio('选择一个名称', ['Alice', 'Bob', 'Charlie', 'David'], index=0)

# 文本输入
st.subheader("输入文本")
text_filter = st.text_input('输入要筛选的文本 (例如: Alice)', '')

# 复选框
st.subheader("启用高级筛选")
advanced_filter = st.checkbox('启用高级筛选')

# 调试信息
st.subheader("调试信息")
st.write("数据框架的列:", df_original.columns)
st.write("类别选择:", categories)
st.write("数值范围选择:", value_range)
st.write("名称选择:", name_option)
st.write("文本筛选:", text_filter)
st.write("高级筛选:", advanced_filter)
...
if start_date > end_date:
    ...
    df_filtered = df_original[
        (df_original['time'] >= pd.to_datetime(start_date)) &
        (df_original['time'] <= pd.to_datetime(end_date)) &
        (df_original['category'].isin(categories)) &
        (df_original['value'] >= value_range[0]) &
        (df_original['value'] <= value_range[1]) &
        (df_original['name'] == name_option)
    ]

    if text_filter:
        df_filtered = df_filtered[df_filtered['name'].str.contains(text_filter, case=False)]

    if advanced_filter:
        # 增加高级筛选条件
        df_filtered = df_filtered[df_filtered['value'] % 2 == 0]

    st.write(df_filtered)

# 确认KBar_dic中存在'open'键
if 'KBar_dic' in locals() or 'KBar_dic' in globals():
    if 'open' in KBar_dic:
        KBar_open_list = list(KBar_dic['open'].values())
    else:
        st.error("'KBar_dic' 不包含 'open' 键")
else:
    st.error("'KBar_dic' 未定义")
